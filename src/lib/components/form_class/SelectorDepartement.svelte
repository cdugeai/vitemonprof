<script lang="ts">
  ...
  let {
    selectedDept = $bindable(undefined),
  }: {
  ...
  let typedText: string = $state('');
  let deptOptions = $derived(
    typedText == ''
      ? deptOptionsInit
      : deptOptionsInit.filter((s) => s.label.toLowerCase().includes(typedText.toLowerCase()))
  );

  let optionDisabled = false;

  let selectedDeptLabel = $derived(deptOptions.find((c) => c.value == selectedDept));

  const triggerContent = $derived(
    deptOptions.find((f) => f.value === selectedDept)?.label ?? 'Selectionnez un département'
  );
</script>

<Select.Root type="single" name="selected_class" bind:value={selectedDept}>
  <Label for="sel-dept" class="px-1">Département</Label>
  <Select.Trigger class="bg-mybeige-bg w-full" id="sel-dept">
    {triggerContent}
  </Select.Trigger>
  <Select.Content class=" max-h-65">
    <Select.Group>
      <Select.Label>Département</Select.Label>
      <Input bind:value={typedText} class="my-1" placeholder="Rechercher" />
      {#each deptOptions as dept_ (dept_.value)}
        <Select.Item value={dept_.value} label={dept_.label} disabled={optionDisabled}>
          {dept_.label}
        </Select.Item>
      {/each}
    </Select.Group>
